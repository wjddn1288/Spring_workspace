...
/*
 	쇼핑몰 어플리케이션에 전반적으로 적용될 수 있는 공통 코드를 AOP의 
...
public class CategoryAdvice {
	...
	public Object getCategoryList(ProceedingJoinPoint joinPoint) {
		// joinPoint 목적 : 원래 호출하려던 메서드를 호출 전, 후에 관여할 수 있는 기능을 지원
		...
		String method=sig.getName(); // 메서드 이름
		...
		try {
			returnObj=joinPoint.proceed(); //원래 호출하려던 메서드 호출 여기서 진행!!
			
			if(returnObj instanceof ModelAndView) { //returnObj의 자료형이 ModelAndView 라면..
