...
// 어노테이션은 jdk 5 부터 지원되는 주석 중 하나로, 우리가 알고 있는 일반적인  주석과 달리
// 프로그램에 사용되는 주석 
@Controller
@Setter
public class EmpController {
	...
	@Autowired
	private EmpService empService; //new 하면 안됨!! 의존성이 강해짐 그러면? 상위자료형으로 가야댐!
	
	
	//CRUD를 이 클래스에서 모두 처리리 할 수 있다
	//ListController, DetailController..등등 따로 만들 필요 없다.
	
	//이 메서드가 어떤 uri를 처리할지 uri 매핑을 걸어야댐!
	@RequestMapping("/emps")
	public String getList(Model model) { //model 데이터를 저장할수 있는 객체
		logger.info("게시판 목록을 처리할 예정");
		
		//3단계 : 알맞는 객체에 일 시키기(서비스에 시키면 됨)
		...
		//개발자가 redirect 를 명시하지 않으면, 디폴트가  forward 이다!!
